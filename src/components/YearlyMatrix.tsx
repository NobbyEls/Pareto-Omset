...
export function YearlyMatrix({ data, year, estimationKey }: Props) {
  ...
  const ytdGrowthFinal = useMemo<{ value: number; label: string } | null>(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      if (prev.value != null && i <= lastActiveMonth) {
        ytdPrev += prev.value;
      }
    }

    if (lastActiveMonth < 0 || ytdPrev === 0) return null;

    // Recompute prev to match exactly the same month range
    let prevSum = 0;
          ...
          {MONTHS_ID.map((m, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Growth column — single merged cell, only rendered on first row */}
                {idx === 0 && (
                  <td
                    rowSpan={13}
